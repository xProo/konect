...
const SUPABASE_ANON_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InliY2NvaGptZGNld2VjZ3praHhtIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTMwOTAzMzgsImV4cCI6MjA2ODY2NjMzOH0.uI1rTqrMkySyFNXxFLNEzsqcHRsy2ms4Ju073wTRDCs";
...
export const database = {
  ...
  // Récupérer les événements d'une communauté spécifique
  async getEventsByCommunity(communityId) {
    const { data, error } = await supabase
      .from("events")
      .select("*")
  ...
  async registerToEvent(userId, eventId) {
    try {
      ...
      return {
        ...
        error: {
          message:
            "Table event_registrations non disponible. Veuillez exécuter le SQL d'initialisation.",
        },
      };
    }
  },
...
export const admin = {
  ...
  async getAllCommunities() {
    try {
      ...
      if (referentIds.length === 0) {
        return {
          data: communities.map((c) => ({ ...c, user_profiles: null })),
          error: null,
        };
      }
      ...
      const eventsWithProfiles = events.map((event) => {
        if (event.communities && event.communities.referent_id) {
          const profile = profiles?.find(
            (p) => p.id === event.communities.referent_id
          );
          return {
            ...event,
            communities: {
  ...
  async searchUsers(query) {
    const { data, error } = await supabase
      ...
      .select("*")
      .or(
        `email.ilike.%${query}%,full_name.ilike.%${query}%,prenom.ilike.%${query}%,nom.ilike.%${query}%`
      )
      .order("created_at", { ascending: false });
    return { data, error };
  },
